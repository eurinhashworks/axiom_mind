...
export const GalaxyView: React.FC<GalaxyViewProps> = ({ ideas, goBack }) => {
  ...
  return (
    <div className="relative w-full h-full bg-axiom-dark animate-popIn">
      <header className="absolute top-0 left-0 p-4 z-10 w-full text-center">
        <h1 className="text-2xl font-bold">La Galaxie</h1>
        <p className="text-axiom-text-secondary">Votre Cortex Visuel</p>
      </header>
      <svg ref={svgRef} className="w-full h-full"></svg>
      <button onClick={goBack} className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-axiom-accent text-white font-bold py-3 px-6 rounded-full hover:bg-axiom-accent-hover transition-transform transform hover:scale-105">
        Retour
      </button>
    </div>
  );
